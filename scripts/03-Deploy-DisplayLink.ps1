...
function Install-IfMissing {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "    [-] $Id already present - skipping."
        return
    }
...
try {
    Write-Host "[+] Phase 3: Standard mode restored. Re-establishing physical network links..."
    ...
    Write-Host "[+] Deploying DisplayLink Core Driver..."
    Install-IfMissing -Id "DisplayLink.GraphicsDriver"

    Write-Host "[+] Deploying DisplayLink Manager from MS Store..."
    Install-IfMissing -Id "9N09F8V8FS02" -ExtraArgs @("--source", "msstore")

    Write-Host "[!] Remediation pipeline complete. Plug in the DisplayLink adapter."
}
# 3. The "Catch" Block: If ANYTHING fails above, execution instantly jumps here
catch {
    Write-Host "`n[X] CRITICAL PIPELINE FAILURE"
    Write-Host "Error Details: $($_.Exception.Message)"
    Write-Host "[!] Manual intervention required for package deployment."
}
